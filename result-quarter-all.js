...
async function loadData() {
  const [participantsSnap, juriesSnap, quarterResultsSnap] = await Promise.all([
    get(ref(db, 'participants')),
    get(ref(db, 'users')),
    get(ref(db, 'roundResults/quarter'))
  ]);

  if (!participantsSnap.exists() || !juriesSnap.exists() || !quarterResultsSnap.exists()) return;
  ...
  const allResults = {};
  const resultData = quarterResultsSnap.val();
