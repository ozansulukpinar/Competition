...
async function generateSemiFinalistsIfReady() {
  ...
  const progressSnap = await get(ref(db, 'juryProgress'));
  ...
  const [r1, r2, r3, r4, participantsSnap] = resultsSnaps;
  ...
  await set(ref(db, 'roundParticipants/round5'), round5.reduce((acc, val, i) => {
    acc[i] = val;
    return acc;
  }, {}));

  await set(ref(db, 'roundParticipants/round6'), round6.reduce((acc, val, i) => {
    acc[i] = val;
    return acc;
  }, {}));
}
